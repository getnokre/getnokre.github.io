...
//! substrate ([render/dom](https://github.com/getnokre/nokre/tree/main/src/render/dom)).
...
pub fn main(init: std.process.Init) !void {
    ...
    try checkQuotation(gpa, sources);
...
/// The colophon quotes the substrate contract word for word, and a
/// quotation is the one claim on this site nothing else checks: the
/// reference gate reads the attribution's *route*, never its words. So
/// the sentence is looked up in the document it is attributed to, here,
/// where every document is already in memory.
fn checkQuotation(gpa: std.mem.Allocator, sources: [][]const u8) !void {
    const i = pages.indexOf("internals.substrates").?;
    const doc = try flatten(gpa, sources[i]);
    const want = try flatten(gpa, content.substrate_quote);
    if (std.mem.indexOf(u8, doc, want) != null) return;
    std.debug.print("the colophon quotes internals/substrates.md, which no longer says it\n", .{});
    return error.QuotationMoved;
}

/// A quotation survives the source's own line breaks, its emphasis
/// markers and its opening capital, and nothing else: whitespace runs
/// collapse to one space, `*` drops out, and case is folded because the
/// quoted sentence opens mid-sentence in the source.
fn flatten(gpa: std.mem.Allocator, text: []const u8) ![]const u8 {
    var out: std.ArrayList(u8) = .empty;
    var space = false;
    for (text) |c| {
        if (c == '*') continue;
...
test "a quotation survives a rewrap and not a rewording" {
    const gpa = std.testing.allocator;
    const a = try flatten(gpa, "with capabilities the Skia\n  *substrate* refuses,");
    defer gpa.free(a);
    try std.testing.expectEqualStrings("with capabilities the skia substrate refuses,", a);
}

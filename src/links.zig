...
fn destOf(gpa: std.mem.Allocator, loc: L.Locale, target: Target) !dom.LinkDest {
...
pub const Resolver = struct {
    ...
    pub fn refs(self: *Resolver) dom.RefResolver {
        return .{ .ctx = self, .resolve = resolveHook };
    }

    fn resolveHook(ctx: ?*anyopaque, em: *dom.Emitter, route: []const u8) anyerror!dom.LinkDest {
...
pub const Live = struct {
    ...
    pub fn refs(self: *Live) dom.RefResolver {
        return .{ .ctx = self, .resolve = resolveHook };
    }
...
const resolver_routes = [_]nok.RouteDef{
    ...
    .{ .name = "internals.dom-substrate", .title = .{ .fixed = "DOM substrate" }, .build = buildLink },
};
...
test "the resolver reads which page a reference is from off the app" {
    ...
    for ([_][]const u8{ "routing", "internals.dom-substrate" }) |page| {
    ...
    try std.testing.expectEqual(pages.indexOf("internals.dom-substrate").?, seen.items[1].from);
...
test "every published document's route name is the one its own path resolves to" {
    var arena = testArena();
    defer arena.deinit();
    const gpa = arena.allocator();
    for (pages.all, 0..) |p, i| {
        if (p.md.len == 0) continue;
        const t = try resolve(gpa, p.md, "docs");
        try std.testing.expectEqual(i, t.page.index);
    }
}

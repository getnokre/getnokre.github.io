...
pub const routes = blk: {
    ...
    for (pages.all, 0..) |p, i| {
        defs[i] = .{ .name = p.name, .title = .{ .of_locale = titleOf(i) }, .for_unresolved = p.kind == .not_found, .build = builderFor(i) };
    }
    const frozen = defs;
    break :blk frozen;
};

pub const comptime_ref = nok.ComptimeRefs(&routes);
...
/// the one claim on it that no route table, audit rule or reference
/// check can reach. `main.checkQuotation` looks it up in the document it
/// is attributed to, on every build.
pub const substrate_quote =
    ...
    "with capabilities the Skia substrate refuses, so long as the semantics " ++
...
fn home(app: *App) !void {
    ...
    try b.styled("A cross-platform app framework whose product is fewer decisions.", .{
        .scale = .h3,
        .ink = .mid,
    });
    try b.text("One app written in Zig, running on macOS, Windows, Linux, iOS, " ++
        "Android and the browser. You say what a screen is and what is on it; " ++
        "nokre derives the accessibility tree, the focus order, the navigation " ++
        "chrome, the layout and the pixels. Every element is semantic, so " ++
        "accessibility is derived from the tree you build rather than annotated " ++
        "onto it — and an automatic audit of twenty-seven rules holds it there, " ++
        "run at init and after every action in every test, through an entry " ++
        "point that takes no skip options at all. The tools go the rest of the " ++
        "way: ARB catalogs the Zig compiler itself validates, a headless e2e " ++
        "harness with byte-exact golden screenshots, and store listings and " ++
        "store screenshots checked against what Apple and Google accept at " ++
        "upload.");
    try b.styled("What the trade costs is expressiveness: text, lines and boxes, " ++
        "grayscale only. Think apps for a grayscale Kindle — and, on the same " ++
        "tree, for a terminal, a watch face, a panel that refreshes one row at " ++
        "a time.", .{ .ink = .dark });
    ...
    try b.spanned(&.{
        ...
        .{ .text = " — one of the decisions nokre does not leave open long enough to get wrong." },
    });
    ...
    try b.spanned(&.{
        .{ .text = "Most of what a UI toolkit hands you is a question — which widget, what color, which breakpoint, how this reads to a screen reader, where this string lives, what a store will accept — and every one of them is also a way to ship a broken app. nokre answers them once, in the framework, and each answer is a guarantee rather than a default. What is left to decide is what your app says and does; the questions, one by one, are the " },
        .{ .text = "introduction", .route = "introduction" },
        .{ .text = "." },
    });

    const promises = try b.stack(.{ .gap = 8 });
    try promise(promises, "Accessible by construction, and gated", "Every element is semantic — a heading is structure, a button is a " ++
        ...
        "cannot be added and cannot be omitted. Malformed structure is " ++
        "rejected at append, so an invalid screen never exists. What " ++
        "construction cannot verify, an audit catches — at init and after " ++
        "every action a test takes, through an entry point with no skip " ++
        "options on it.");
    ...
    try promise(promises, "Tooled from the first string to the store page", "ARB catalogs compile at comptime, which makes the Zig compiler the " ++
        "validator: a missing key or a plural branch a language never selects " ++
        "is a build error, not a blank on somebody's screen. A headless e2e " ++
        "framework drives the real app through the real event pipeline and " ++
        "takes byte-exact golden screenshots — no browser driver, no window. " ++
        "Packaging writes the icons and the link preview, checks a store " ++
        "listing field by field against what each store accepts, and captures " ++
        "the screenshots at the sizes it publishes.");
    ...
    try b.text("Most of these are load-bearing for a promise above: remove one and " ++
        ...
        "needed answered — which is the product, not a side effect of it.");
    ...
    for ([_][]const Span{
        ...
        &.{
            .{ .text = "No device-conditional affordances. ", .strong = true },
            .{ .text = "There is no call that asks what you are running on, and none is coming. An accelerator may only accelerate a control that is already on screen, focusable and announced — a device may change how a control is reached, never whether one is there." },
        },
        &.{
        ...
        &.{
            .{ .text = "No named platform groups. ", .strong = true },
            .{ .text = "No \"phones\", no \"tablets\", no \"wearables\" to test against. A group goes stale the first time a device is unlike its group — a foldable, a tablet with a keyboard, a car head unit. What nokre reads instead is a fact about the surface in front of it: how big it is, and whether it clips or reflows." },
        },
    }) |item| {
    ...
    try b.spanned(&.{
        ...
        .{ .text = " have these problems, because the framework cannot express them. The argument, in full, is the " },
        .{ .text = "introduction", .route = "introduction" },
        .{ .text = "." },
    });
    ...
    try b.text("Five native shells are working: window, input, IME, clipboard, and " ++
        "a screen reader on each. The sixth platform, the web, has no shell " ++
        "because the browser is one — there the tree is written as markup and " ++
        "the accessibility tree is the page itself. What differs at a glance " ++
        "is the text scaler and the accessibility backend.");
    ...
    try b.spanned(&.{
        ...
        .{ .text = "A page that fails the audit is not published: the build stops instead. " },
        .{ .text = "What that means, and where the substrate stops short, is the " },
...
fn gallery(app: *App) !void {
    ...
    // The pair closing this page's title is the `header_action` specimen, and
    ...
    try note.styled("The controls below are specimens. They are drawn the way nokre " ++
        ...
        "nothing: on this substrate there is no app behind them.", .{ .ink = .mid, .scale = .small });
    ...
    try b.heading(.h3, "quantity");
    try b.quantity(.{
        .value = "1,284",
        .unit = "members",
        .caption = "Across every workspace you belong to",
    });
    try b.styled("The one number a screen is about, drawn as the figure it is — where meter is words with a bar restating them and badge is words with a border, here the number is the content. nokre formats no number: grouping, decimal marks and digit shapes are the locale's, so the value arrives as bytes and this only places them. The figure draws at the h1 scale in the mono family so a column of these lines up digit under digit, and the unit sits on its baseline — before it under RTL, because the number leads and the unit trails. Where the pair will not fit the span the unit takes a line of its own, and the element owns that decision.", .{ .ink = .mid, .scale = .small });
    ...
    try waiting.heading(.h3, "Engineering");
    try waiting.text("Everyone here, and how far each of them is through the sweep.");
    // Two rows whose names have not arrived. A pending name is in no
    // accessibility tree and no focus order, which is what lets rows repeat
    // under one heading without colliding on a shared name.
    const waiting_rows = try waiting.tileGroup(.{});
    try waiting_rows.tile(.{ .label = nok.pendingValue("ada@example.com"), .detail = "Collected", .route = "gallery" });
    try waiting_rows.tile(.{ .label = nok.pendingValue("bob@example.com"), .detail = "Two keys out", .route = "gallery" });
    const waiting_acts = try waiting.stack(.{ .axis = .horizontal, .gap = 8 });
    try waiting_acts.button(.{ .label = "Collect keys" });
    try waiting_acts.button(.{ .label = nok.pendingChars(9) });
    // The filled field is where the web substrate leaked worst: it sets a
    // text-fill colour later in the sheet than the scope's own class. Under
    // the scope it takes no keystroke either.
    try waiting.textInput(.{ .label = "Filter", .value = "engineering" });
    try b.styled("A scope whose content has not arrived, laid out exactly as the stack it replaces. Under it the screen's ordinary draw code runs and its words are its own: headings, labels, prose, tile and button words are real text, announced as text. What blocks is a string handed to pendingValue — a muted block one reference advance per character of the example that sized it, silent to assistive tech, refused outside a scope, and never carrying the example's words into the tree. That is the inversion of the rule this element shipped with, and the frames decided it: a screen where no word is real reads as a rendering failure, while a screen that is itself with two values still arriving reads as a wait. Every control here is busy — real words, no press, no keystroke, focus stop kept — and the one drawn muted is the second button, whose own label is what is pending. A control that must stay live sits above the scope. There is no motion; the shape is the whole of it.", .{ .ink = .mid, .scale = .small });
    ...
    try named.button(.{
        .label = "Remove",
        .accessible_name = "Remove \u{2014} ada@example.com",
        .form = .{ .secondary = null },
    });
    try named.button(.{
        .label = "Remove",
        .accessible_name = "Remove \u{2014} bob@example.com",
        .form = .{ .secondary = null },
    });
    try b.styled("A column of rows each ending in the same verb: \"Remove\" is the right thing to read beside every row and an ambiguous thing to say, so the audit's uniqueness rule refuses it. The name disambiguates without putting the disambiguation on screen. It never replaces the words — WCAG 2.5.3, so a voice-control user can say what they can see — and append refuses a name that does not contain them.", .{ .ink = .mid, .scale = .small });
    ...
    try b.heading(.h3, "ranking");
    try b.ranking(.{
        .label = "Where a notice goes",
        .options = &.{ "In the app", "By email", "By push", "Nowhere" },
        .viable = 2,
        .viable_min = 1,
        .viable_max = 3,
    ...
    try group.tile(.{
        .label = "Static sites",
        .detail = "The DOM substrate, and what it refuses",
        .badge = "New",
        .route = "static-sites",
    });
    ...
    try b.spanned(&.{
        .{ .text = "The two glyphs closing this page's title are its " },
        .{ .text = "header_action", .code = true },
        .{ .text = "s, and they are the specimen — one may only stand at the root of a screen, so there is no row of them below to show. A screen states one or two; the tree refuses a third, because the header is one line and its middle is the title. A pair stands flush: each glyph already sits in the middle of its own target, and a gap between two targets is a place a thumb lands on neither. They are the screen's own content, cleared by a navigation, where the nav above is chrome and survives one. The other end of the same line is the " },
        .{ .text = "back", .code = true },
        .{ .text = " control, which needs a stack to point at and so cannot appear on a site whose every link is an arrival." },
    });
    ...
    try b.spanned(&.{
        ...
        .{ .text = " and the select picker are opened by an event — a press, an arrival, a change. This substrate has no events, so they cannot appear here. They are specified in " },
...
fn colophon(app: *App) !void {
    ...
    const quote = try b.blockquote();
    try quote.text(substrate_quote);
    try quote.spanned(&.{ .{ .text = "— " }, .{ .text = "internals/substrates.md", .route = "internals.substrates" } });
    try b.text("That document splits the guarantees in two. Grayscale, CPU raster, " ++
        "pixel determinism and byte-exact goldens belong to the Skia substrate. " ++
        ...
        "whatever renders it. This substrate keeps the second list whole.");

    try b.heading(.h2, "What this substrate keeps");
    ...
    for ([_][]const Span{
        &.{
            ...
            .{ .text = "Layout is computed and then ignored. Pixel determinism is the Skia substrate's promise, not this one's, and a page that fixed its line breaks would break the first reader who changed their text size." },
        },
        &.{
    ...
    try b.text("Documentation pages are nokre's own Markdown, rendered by nokre's " ++
        ...
        "fails the build the way ComptimeRefs fails an app that spells one out.");
    ...
    try pageTiles(app, b, &.{ "internals.substrates", "internals.pixel-model", "markdown" });
}
...
test "the footer is a stack, not a landmark this substrate has no element for" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();

...
The website for [nokre](https://github.com/getnokre/nokre), the
cross-platform app framework — generated by nokre.
...
And then the app ships on top of the pages it wrote. The DOM substrate has
...
`docs/internals/substrates.md` left this door open on purpose:
...
> with capabilities the Skia substrate refuses, so long as the
> semantics — element set, behavior, focus model, a11y tree — are
> conveyed faithfully.

`/en/colophon/` is the long version, including the two places this
substrate stops short.
...
`/en/internals.dom-substrate/` — and there is one locale today. The
...
A document that is renamed takes its URL with it. There is no server
here and no redirect, so an old address falls to the 404 body, and
`failOnStale` names the directories the old route left published until
they are deleted. That cost is why a rename waits for a name that has

...
| [src/services/roaming_store/roaming_store.zig](../../src/services/roaming_store/roaming_store.zig) | the same pouch with the opposite posture — entries meant to reach the account's next device, namespaced by `pkg_id` plus a suffix ([roaming_store.md](roaming_store.md)) |
...
- **Which app runs is decided by which artifact was installed, and that
  was decided by which `addApp` call built it.** `dwell` is a build-time
  declaration: it is recorded in `build_surface.txt`, checked once (two
  apps of one package may not claim the same one), and compiled into the
  app as `nokre.declared.dwell`. Nothing reads it at run time today.
  Every shell nokre ships boots exactly one app per process or module
  instance, by recorded decision — the web service legs share a
  single-app anchor, `workers/post.zig` and the live driver hold module
  state on it, and `NokreActivity.onConfigurationChanged` says the app
  lives once per process the way the web shell's module does. Two apps
  in one package are two artifacts — two sites, two Activities, two
  executables — never two `App`s in one process. The contract a future
  glance shell reads is that one symbol: before booting, it may ask
  `nokre.declared.dwell` and refuse an app that says it is not for this
  device, rather than drawing a desktop screen on a watch. nokre makes
  no such refusal itself — there is no glance substrate to make it on,
  and inventing a launcher for a device that does not exist is how
  speculative abstractions happen.
...
permit non-Skia substrates is [substrates.md](substrates.md); the one
that exists is [dom-substrate.md](dom-substrate.md).

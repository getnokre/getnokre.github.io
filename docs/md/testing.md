...
`.medium` says what the surface this test stands the app on does with a
row too wide for it — a harness is a driver, and this is the driver's
word, said at `init` so the first screen is built and audited under it
rather than under the default. An app may only be stood on a medium it
declared ([The audit matrix](#the-audit-matrix)).
...
- **The name wins, and the words are the fallback.** A query is handed
  one string and, where a `button` states an `accessible_name`, two
  things could answer it: the name and the words drawn on the pill. The
  name is searched first and whole — it is what assistive tech
  addresses and what the audit holds unique, so it is the only one of
  the two guaranteed to name a single control. The words are tried only
  when nothing answers to the name, and then **only if exactly one
  element on screen draws them**: the words are precisely the string a
  roster repeats, and returning the first of five "Remove"s would drive
  the wrong row silently. So `press(.button, "Remove — bob@…")` reaches
  its row; `press(.button, "Remove")` reaches the lone Remove on a
  screen that has one and finds nothing on a roster that has five. A
  string that is one control's name and another's drawn words resolves
  to the **name**. The `get*` listing prints both where they differ.
- **A stand-in answers for itself and hides the template it draws.**
  Inside a `stand_in` ([elements.md](elements.md#stand_in)) every walk
  drops the subtree. This survived the inversion of that scope, and it
  is deliberately **not** what the a11y snapshot does — a screen reader
  hears the real words under the scope, because they are real. A
  locator is not a reader: it is the front half of a driver and every
  verb it feeds *acts*. A screen that is still waiting takes no press
  and no keystroke, so a locator that returned the template's field
  would hand `clearField` a control the app refuses to edit at the
  moment the honest answer is "not yet". Dropped, it is simply absent,
  so a driver's wait keeps waiting and lands on the real field when the
  phase flips.

  The **scope itself stays queryable**, and that half is deliberate: it
  is one live status saying its `label`, the only thing a screen reader
  is told while the wait runs, so `queryByLabel("Loading…")` finds it
  and `untilGone("Loading…")` is the wait for the phase flipping. It
...
`edgePanBack` is the whole back *gesture*: in from the leading edge,
past the threshold, released
([routing.md](routing.md#the-back-gesture)). A test about the
*threshold* rather than the navigation dispatches `.edge_pan` steps
itself and reads `knocks()`, the journal of haptic crossings the gesture
fired; a press crosses nothing and fires none.

`back()` is a named `press` rather than a fifth ladder, and it takes no
argument for the reason it exists: nokre owns both halves of that
control's identity — the `.back` role and the word `App.setChrome`
writes into it — so a caller spelling the pair itself is right in
English and wrong in every other language its app ships.
...
setup at all — a bare `HarnessApp.init` already gives it somewhere to keep a
...
verb (followed by `app.runtime.pumpToQuiescence()`, which the harness verbs do
for you).
...
verb (followed by `app.runtime.pumpToQuiescence()`).
...
- `expectDisabled(label)` — a control that declines rather than acts,
  ...
  **by label across element kinds**, the way `expectBusy` is: it reaches
  all nine kinds that carry a `disabled`
  ([elements.md](elements.md#turning-a-control-off-disabled)), and reads
  the state by reflection over that field rather than off a list — a
  list is what left this verb buttons-only while two fields already
  carried the state. An element that cannot be disabled at all —
  `copyable` is the only interactive one left — says so rather than
  reporting a placid "enabled".
...
- `expectBusy(label, expected)` — a control with work in flight: the
  state `in_progress` draws and a `nokre.Latch` produces. Located **by
  ...
  **`DriverApp` has no twin for this one**, and the omission is the
  ...
  *holds*. A live driver holds nothing: the work may finish before the
  ...
  the `DriverApp` section's wait rule was written to end. A scenario
...
capturing proves it said something a reader could act on. The sink a
capture installs is process state, so `said.start()` is a compile error
outside `zig test`: a driver that installed one would route its whole
report into a buffer nobody reads and exit non-zero having printed
nothing.
...
**An audit finding must be invariant per (app, medium), and never across
the viewport.** A rule may read `App.medium`: an app declares which
mediums it serves (`AppOptions.mediums` in its build file) and the
matrix runs once per declared one, so a finding that holds within a
medium is one a consumer can act on. A rule may not read `App.viewport`,
which nothing declares — a finding that fires at one window size and not
another is unfixable. Nothing in the audit reads either today; the
clean-clip exemption is the rule that had to say which of the two it was
exempting for ([accessibility.md](accessibility.md)).

**The declaration is a bound, not a label.** `HarnessApp.InitOptions`
takes a `medium`, and standing an app on one its build file did not
declare is `error.UndeclaredMedium` — the diagnostic names what the app
did declare. That is the only run-time meeting between the declaration
and `App.medium`, which stays the driver's word; the other is at build
time, where a wasm artifact whose app does not declare `.reflows` is
refused outright. Its gate is a test root of its own,
...
width either example is stood at, so the row's shape answers the same
under either medium. What the axis *is*, here, is the mechanism a
consumer with a crowded roster and two declared mediums gets — bounded
by their declaration and by nothing else. The width axis is a real
second stand and it is new: before it, every screen of every example was
audited at exactly one size. Neither proves pixels — that is the
goldens' — and neither proves anything about a medium the app did not
declare, which is the bound working.

**The medium mints no goldens, and that is a decision.** Golden paths
gain no medium segment: an automatic one would rename every committed
baseline to say what none of them needs to say, and a conditional one
would make a file path depend on a build declaration, orphaning a whole
set the day `mediums` is edited. A consumer's set multiplies where they
author it instead — `.medium = .reflows` in the init options and the
varied fact in the golden's own name, exactly as `-rtl` and `-dark`
already do.

nokre's own corpus grows by zero on this axis, and the census is the
argument. Only a golden carrying a nav could move under the medium;
four of those are wider than `metrics.sheet_max_w`, and in all four the
roster already stands as a row under `.clips`, so a `.reflows` twin
would be byte-identical. And a golden that *would* differ cannot
honestly exist: the medium only changes frames where the row is wider
than the frame, which is a roster hanging off both edges — a browser
wraps it, and a rasterizer has no wrap to draw, so the picture a
reviewer would be approving is a roster cut in half at both ends. The
medium's effect on the **tree** is pinned where it is exact instead: two
a11y readings of one roster at one width, one per medium, in
`harness_test.zig`.

One gap is stated rather than implied. Every document the DOM substrate's
markup gates build runs at `.reflows`, but not one of their fixtures
carries a nav wide enough to collapse — so the `.reflows` row is emitted
by that path and asserted about nowhere. The harness pair above is the
only place either shape is pinned.
...
6. A take at `.medium = .reflows` photographs the tree a browser would
   *wrap*, and a rasterizer has no wrap to draw — so a roster wider than
   the frame will look cut in the picture. Keep such a take at a width
...
`docs/` is prose, and prose has no compiler. The one claim in it a machine can
settle is whether a Markdown destination names a file that exists, and until
`tests/doc_links.zig` nothing in this repository read `docs/` at all: the site's
link gate lives in the other one, behind a rebuild from a sibling checkout, so
someone editing nokre alone got a green `zig build test` and found out later,
somewhere else. A rename that moves a filename through a hundred references is
exactly the change that falls into that gap.

The walk covers every `*.md` under `docs/` and `docs/internals/`, and asserts
two things. Every `](…)` destination that looks like a path resolves against the
directory of the file it was written in — an in-app route (`terms`,
`ticket~2938`) and an external scheme are addresses rather than paths and are
skipped, which is the site resolver's business. And every document is linked
from its own track's README, the consumer index for `docs/*.md` and the
contributor index for `docs/internals/*.md`, so a new page cannot be published
and unreachable at the same time.

Two things it deliberately leaves alone. **Fragments** are the site's: it
resolves them against real rendered headings with nokre's own parser, and a
second slugger here would be a second copy of one rule. And a destination inside
a **comment** — a `docs/internals/…` path cited from Zig or JavaScript — is read
by nothing, here or there; grep is the tool for those, and they are what to
check after a document is renamed.

It is wired with `setCwd(b.path("."))`, because a run's working directory is the
cache directory otherwise, which is where every relative path in it would
resolve.
...
- **The shell.** The driver delivers events to `App.dispatchInput` and reads
...
- **the boot's two writers** — `packaging.webBootJs` writes the shell's
  `boot.js` and `live-boot.js` reads a generated page's block, and both
  stay, because `mount` reads an *absent* `content` and `locale` as
  answers a shell must give and a `null` is not the same value. Two
  writers of one call is a drift waiting to happen, so the scenario is
  a diff: both files, verbatim from the site, run against a `live.js`
  that records the call instead of making it, and the two option
  objects are compared key by key. The generated call must be exactly
  the page's block with its two ids resolved to elements; the shell's
  must be that call minus a closed list of facts it has no page to
  state, every one of which the page in the scenario states. A key on
  one side alone — an option added to `BootConfig` and not to
  `webBootJs`, a `locale: null` typed into either file — fails here and
  nowhere else, because a Zig test sees each writer's bytes and never
  the call they make.
...
desktop platforms. The nav and reserve scenarios are the nearest thing to
...
- **`HarnessApp` stays inside `zig test`.** Its whole value is that every
...
- **The driver layer already is that seam.** `testing.driver`,
  ...
  `testing.golden` name no `builtin.is_test` at
...
The `Pacer` a *native* driver wants is shipped too, because there is
only one of it: `wait.Clock` reads the app's own clock service for the
time and naps on the `std.Io` the process was started with. Both are
facilities a driver already holds, and a third time source beside them
would be a second clock in one run. It hands its `Pacer` out through a
**mutable** receiver on purpose. A `Pacer` carries its context as a bare
pointer, so the shortest way to write the call — `(&Clock{…}).pacer()` —
would leave every poll reading a dead frame; `pacer(self: *Clock)` makes
that spelling a compile error instead, and storing the `Clock` beside
the `DriverApp` leaves one lifetime rather than two:

```zig
var clock: nokre.testing.wait.Clock = .{ .app = &app, .io = io };
var d: nokre.testing.DriverApp = .{ .app = &app, .pacer = clock.pacer() };
```

A driver that wants a different deadline or a different poll interval
names it there. Both are optional, and null means "whatever `Pacer`
declares", so neither figure is written down twice.
...
| `untilDestination(app, pacer, title)` | the nav can answer for that destination — the row's `nav_item`, the `nav_here` marker, or the collapsed chip standing in for a roster that carries it |
| `untilEnabled` / `untilDisabled(app, pacer, label)` | that control — button or text field — takes input again, or has stopped |
...
One shape sits *on top of* `Ready` rather than beside it, and it ships
because both drivers wrote it before nokre did.
`wait.waitUntilHeld(app, pacer, what, ready, hold_ms)` ends only when
the predicate has answered true **continuously** for `hold_ms`, and
starts the hold over the moment it answers false. The reason is worth
keeping even if you never use the verb: work reaches the delivery queue
one pump *after* the press that asked for it, so a single quiet poll is
as likely to be the gap before the work as the end of it. A queue that
reads empty because nothing has been enqueued yet is the shape of a
test that passes until the machine is busy. The predicate stays yours —
a proof queue draining, a prefetch sweep finishing; only the shape is
nokre's, and `DriverApp.waitUntilHeld(what, ready, hold_ms)` is the same
wait with the pacer already bound.
...
### `DriverApp`: the harness's verbs over a live app

`testing.DriverApp` is what a driver holds where a test holds a
...
omission until you ask what it would mean: it is `expectAbsent` plus a
wait, and under the mocks nothing waits. The two would be one verb with
two names there, which is the thing this tier exists to avoid.
`edgePanBack` is absent for a different reason, and it is why `back` is
the verb that crossed: what a *gesture* verb is for is the gesture's own
behaviour — the threshold, the crossings — and the only thing that
observes those is `knocks()`, the mock haptic's journal, which a live
app has no counterpart to. A driver that means "go back" means the
control. One that genuinely wants to synthesize the drag still has
`testing.driver.edgePanBack` over its own `*App`, without this tier's
audit. A third substrate's driver is a `DriverApp` and a domain vocabulary,
not a third transcription of this file.

```zig
pub const DriverApp = struct {
    app: *nokre.App,
    pacer: nokre.testing.wait.Pacer,
    notes: ?nokre.testing.driver_app.RefusalNotes = null,
    refusals: ?nokre.testing.driver_app.RefusalLog = null,
};

var d: nokre.testing.DriverApp = .{ .app = &self.app, .pacer = clock.pacer() };
...
- **`submit` is the one verb with no `HarnessApp` twin, and that is not
  `expectBusy`'s omission.** It puts the caret in a named single-line
  field and dispatches Enter, so `TextInput.on_submit` fires through the
  exact path `editing.handleInputKey` gives a real key — which is the
  path the mobile shells map the return key to. A unit test needs no
  such verb: it holds the node and presses Enter against it. A driver
  knows only the words on the screen. It settles the way `press`
  settles and waits for nothing afterwards: the return key is a press,
  and a wait on the far side would be waiting on whatever the app does
  next rather than on the key. Two refusals are named rather than left
  to a deadline — a disabled field, and a field whose Enter does
...
- **`expectInFrame(role, label)` asks the frame, not the future**, and
  it exists because of a bug that shipped in a driver. A wait asks the
  tree at the moment it stops waiting; a capture asks the pixels a
  second later. In between, a list arrived, the layout re-ran, and a row
  of actions too wide for the device folded its tail behind the
  chrome's overflow control — so a walk that had waited for a button
  photographed a screen that no longer had one. This verb answers the
  question the walk was actually asking: *would the frame about to be
  written show this control*. It distinguishes "folded away at this
  width" from "not on this screen at all", names the fold's control and
  the viewport in the refusal, and separately catches a control the
  viewport cuts off. Like `expectAbsent`, it deliberately does not wait
...
- **`expectDestination(title)` asks the nav, not a role.** It is the
  ...
  `expectPresent` with a role filled in, because which of the three
  shapes the bar wears — the row's `nav_item`, the `nav_here` marker for
  the screen already under foot, or the collapsed chip standing in for
  the whole roster — is layout's decision, so a driver that names one is
  asserting against a viewport width rather than against the nav. Nor is
  it `untilLabel`, which a heading carrying the same words would satisfy
  just as happily. The chip clause reads the roster: a bar that has
  collapsed answers for a destination only if one by that name is
  actually on it, or the verb would report that the bar collapsed and
  nothing about the destination. It has a harness twin, and the three
  shapes live in one place for both of them — `driver.hasDestination`,
...
- **Every other verb waits for the thing it acts on or asserts
  ...
  **value**, `expectProblem` for the **reason**, and
  ...
  hands it back is in flight. Besides the two assertions above, the only
  verbs that do not wait are `auditFrame` and `settleFor` — one audits the
  frame an action produced, the other is a bare deadline.
...
**One closed error set, because the drift came back.** `DriverApp` exists
because two shipped drivers had each re-implemented the same twenty-odd
verbs from the primitives, under names and algorithms that had drifted
apart. Moving the verbs here fixed that — and the drift reappeared one
layer up. Every verb inferred its own error set, so every consumer
wrapped every verb in a three-line forwarder to convert the error and
log the refusal, and twenty-odd forwarders apiece is simply another wall
to drift. Both walls had. Worse, **seven verbs were reachable from
neither**, `expectValue` and `expectProblem` among them — the two this
document singles out as mattering most against a real server. A verb
nobody had wrapped was a verb nobody could call.

So every verb answers `DriverApp.Error`, three tags and closed, which a
driver can name in its own signatures; and the transcript line gets a
seam of its own. Together they leave a forwarder nothing to do. Three
tags, because three conditions genuinely differ:

| tag | what happened |
| --- | --- |
| `Refused` | the app did not do, or does not show, what the verb asked |
| `OutOfMemory` | a verb that allocates could not — nothing was asserted and no screen refused anything |
| `TraceFailed` | a step observer could not write |

One tag for every refusal costs nothing, because the tag was never
carrying the diagnosis: nokre has already printed the reason and the
screen before the verb returns, and the seam below gets the original
error anyway. The other two are separated because a driver acts on them
differently. `OutOfMemory` is the process running out, which is a reason
to end the run rather than to record a failed step — a scenario that was
never run is not a scenario that failed. `TraceFailed` is the instrument
breaking, not the app: whatever the scenario was proving is still true,
and there is no screen to blame for it.

**Two seams, and binding them is what a driver writes instead of the
wall.** Both are `{ ctx, call }` pairs that
[`bindAs`](elements.md#binding-callbacks-nokre-never-sees) fills from an
ordinary method. `DriverApp.notes` is for what nokre cannot know: bind it
and it prints on any refusal — the proofs still queued, the load phases
behind a screen that never filled in. nokre says what is on the screen;
the driver says what is behind it. `DriverApp.refusals` is what the driver
*does* about a refusal: count it, name it, put a line in its own
transcript. It is handed what was attempted — the verb and what it was
aimed at, in your own spelling, `press "Archive"` — and the original
error, *before* the collapse into `DriverApp.Error`, which is the one fact
the closed set gives up. Those words are borrowed for the length of the
call, like every other diagnostic here: print them or copy them, never
keep the slice.

The order is fixed, and worth knowing because a driver reads its own
transcript by it. On every refusal, exactly once, whichever verb raised
it and whichever step of the verb did:

1. whatever nokre itself has to say — for almost every refusal that is a
   stated reason and the whole screen, printed by the tier that refused
   before the verb returns;
2. `notes`, what is behind that screen;
3. one line naming the verb and the error, `press "Saved" refused:
   WaitTimeout`. It is printed whether or not anything is bound, and it
   is the floor that makes collapsing every refusal into one tag honest:
   a few paths under these verbs raise without a word of their own, and
   a closed error set over a silent path is a verb that failed for no
   stated reason;
4. `refusals`, the same two facts for your transcript.

Two verbs a driver keeps for itself. `settleFor(millis)` ships (it is a
...
`settleFor` is `wait.settle(app, pacer, millis)` bound to the driver's own
app, and the free function is there because a driver's *other* app
usually has no `DriverApp` on it: the one that stands up fixtures over the
same transport has no screens to drive, and still has to let what it
started land. That is the same fact the allocator rule turns on — the
native transport's request threads are detached and never joined
([internals/http.md](internals/http.md#the-transports)) — so a run that
stops asking while work is in flight loses whatever the server was about
to write.
...
A driver whose `main` is `entry.run` ([below](#the-process-a-driver-is))
does not write that block at all: the install is transitive, because
reaching a module is what pulls its `export`s into a binary. It stays
the driver's own line when the driver builds its own process shape.
...
the real shell's collide at link time, which is
exactly the guard. It also owns no loop — pumping `app.runtime.pumpDeliveries()`
...
worked example of the whole shape, and it runs on every `zig build test`
on a desktop POSIX
...
### Holding an answer the screen never shows

Some of what a scenario asserts never reaches a screen. A driver calls
one of its own ports — a fixture it seeds, an admin endpoint, the API
the app itself talks to — and is answered in a callback. Two things
stand between that callback and an assertion, and neither of them names
anything of a domain: the borrow a callback grants ends when it returns,
and nothing has pumped the delivery queue that will carry it. Both
shipped drivers wrote that pair by hand. `testing.results` is it, once.

```zig
const landing: nokre.testing.results.Landing =
    .{ .gpa = run.gpa, .app = run.app, .pacer = clock.pacer() };

// `circles` is the driver's own port; the pair it takes is the one
// `Waiter` fills — `{ ctx, call }` over `fn (?*anyopaque, Result) void`.
var w = landing.waiter(Circles.Result);
circles.load(.{ .ctx = w.issue(), .call = @TypeOf(w).on });
const held = try w.wait("the circle list");
```

`Landing` is the three things a waiter needs — somewhere to put the
copy, the app whose queue delivers the answer, and the clock the
deadline is measured on — and it vends two of them: `waiter(R)` for a
port that answers `R` outright, `answers(Result)` for one shaped
`error{OutOfMemory}!Result`, where an allocator that would not answer
says nothing about the call and so stays the driver's own failure rather
than becoming a result. The copy underneath is `results.dup`, generic
and deep — the driver tier's answer to the rule a screen answers with
`Str` and `Rows`
([elements.md](elements.md#holding-what-a-callback-borrowed)). It
walks structs, optionals, tagged unions, arrays and slices, and refuses
at compile time the three shapes it cannot copy honestly — an untagged
union, a sentinel slice, and a pointer that is not a slice. It is public
for a driver holding an answer neither waiter shapes, as is
`results.whenSet(flag)`, the `Ready` that watches a bool a callback sets.

**The address of a waiter is the callback context**, because Zig has no
closures. That is the one rule this module cannot enforce and the one it
works hardest to catch: `issue()` is the point at which the address
becomes load-bearing — it stamps where the callback was told to write —
and `wait` refuses at any other address with `error.WaiterMoved`
*immediately*, instead of burning a full deadline on a flag that was set
somewhere else. Hand the port `w.issue()` rather than `&w`, and never
move or copy a waiter between the issue and the wait. The same check
catches the mistake from the other end: a waiter that was never issued
at all, whose port was handed nothing, fails on the spot rather than at
the deadline.

The pump is `wait.waitUntil` with a bound predicate, deliberately — one
loop, one deadline, one clock for the whole driver tier. The cost of
that is stated rather than hidden: a port result that times out prints a
screen dump under it, and on the screenless API-side app a driver often
keeps for exactly these calls, that dump is two lines.

### The process a driver is

`main` is the last place a driver still writes nokre's own facts by
hand, and five of them across the two shipped consumer trees were the
same program: an arena chosen for one specific reason, one positional
argument or a usage refusal, a headless `App` at a fixed address, and a
▶/✓/✗ transcript around a single call. Every one of those is a fact
about nokre. Not one of them is about a scenario.

```zig
pub fn main(init: std.process.Init) !void {
    var state: State = .{ .io = init.io };

    // The platform half stays here, where the allocator is.
    var argv: std.ArrayList([]const u8) = .empty;
    var it = try init.minimal.args.iterateAllocator(init.arena.allocator());
    defer it.deinit();
    while (it.next()) |arg| try argv.append(init.arena.allocator(), arg);

    try nokre.testing.entry.run(argv.items, .{
        .usage = usage,
        .app = .{ .viewport = .{ .w = 390, .h = 844 }, .ctx = &state },
        .about = "against http://localhost:8090",
    }, nokre.bindAs(nokre.testing.entry.Job, State.run, &state));
}
```

`run` owns the process arena; the decision that anything but exactly the
declared arity is a misuse, which prints the usage text and exits 2
(`.arity = .none` for a driver that does one job, whose name then comes
from `.name`); the headless `App`, at the fixed address a driver owes it;
the ▶/✓/✗ transcript, timed through the app's own clock service, because
a driver already links that one OS call; and the exit codes — 1 and a
`✗` line on a run that returned an error, a normal return and a `✓` on
one that did not. The job is handed a `RunContext`: the
arena, the `*App`, the positional, and a `reported` counter whose
meaning is entirely the driver's. nokre never reads one into it and
never writes to it; whatever stands there when a job fails is printed as
`N assertion(s) reported`, and a driver that counts nothing leaves it at
zero.

**The allocator is the fact worth stopping on.** A driver's allocator
must not be leak-checking, and the reason is nokre's own rather than a
matter of taste: the native http transport runs one *detached*,
never-joined thread per request, and a thread still running at exit is
still holding its buffers —
[internals/http.md](internals/http.md#the-transports) has why the
transport is built that way and what it therefore costs a run that
checks for leaks. Five consumer driver files rediscovered it
independently. `run` settles it
with an arena released when the process ends, which is the whole memory
story a run needs — it is minutes long and it ends when the process
does, so anything allocated from `RunContext.gpa` may be held for the whole run
with no matching free. It is specifically not the leak-checking
allocator `std.process.Init` hands you for temporaries.

**And that arena is never deinit'd, which is the other half of the same
fact.** Freeing it hands the pages back while a transfer thread that
cannot be joined still points into them; `run` therefore has no `defer`
on it, and a driver building its own process shape must not add one. The
defer that used to be there ran on exactly one path — the passing one,
since both refusals exit the process — so the crash it caused always
arrived *after* a green journey, which is the worst place for a crash to
be.

**`argv` is a slice, not an iterator**, which is the platform half
handled rather than warned about: Windows and WASI need an allocator to
iterate at all, and on Windows the iterator's buffer is reused per
`next`, so a positional read out and kept is dangling by the following
call. That is the real reason all five shipped drivers duplicated their
positional before using it. Building the slice in your own `main`, where
the allocator already is, removes the hazard rather than documenting it.

What `entry` refuses falls on the far side of the line
[static-sites.md](static-sites.md#which-side-a-new-question-falls-on)
draws for a generator, for the same reason: nokre can check an answer to
none of it. Which runs exist and what dispatches between them; the
*content* of the usage text — a required field, since nokre cannot write
it and the run cannot go without it, the way a published site's origin
is required and unknowable at the same time; every environment variable
and base URL; what a reported assertion means; and the post-run drain,
whose length is a fact about the servers a driver talks to.
...
`DriverApp.startTrace` is the same seam a `HarnessApp` has, one tier up, and
...
same verbs — a `DriverApp` scenario's trace reads like a unit test's.
...
try d.expectInFrame(.button, "Continue");    // would the next frame show it?
try d.press(.button, "Continue");            // 0002-press-Continue.*
_ = try d.untilLabel("Welcome back");        // 0003-until-Welcome-back.*
try d.traceStep("after the reply landed");   // 0004-after-the-reply-landed.*
```

**The waits write steps too, and that is not a convenience.** An action's
step is taken after the action, so a screen that loads and is then
pressed is photographed *loading* — by the press that navigated to it —
and *busy*, by the press that acted on it, and settled in neither. The
screen the second press aimed at existed only in between, which is
exactly where a wait ends. So every screen-facing wait — the `until*`
family, `waitUntil`, and `expectGone`, which is `untilGone` under its
assertion's name — writes a step of its own, named for the wait and
numbered in sequence with the actions.

Only when the tree moved since the last step. The digest of what the
tree sink would write is kept per step (`trace.digest`), so a wait that
resolves on the screen already on file writes nothing rather than
filing the same frame under a second name — and hashing the dump rather
than walking the tree a second way is what keeps the two from ever
disagreeing about what a change is. `waitUntilHeld` is the one wait
that stays silent: what it holds out for is background work with
nothing on screen to show for it, so the screen it settles belongs to
whichever verb names it next.

`startTrace` takes one observer, so a run with a tree sink and a frame
sink needs a `trace.Tee` over both — and a `Tee` borrows the array of
observers it fans out to, which is a lifetime callers kept getting right
by hand. `TraceInstruments` is that array and that `Tee` as fields of the one
value you already store: hand it a directory for the trees or null,
an already-built frames observer or null, and `observer()` composes
whichever exist. Neither is an error — a run with no sinks at all gets a
`Tee` over nothing, which writes nothing and cannot fail, so a driver
that traces conditionally still passes an observer rather than branching
at the call site. `observer()` takes a mutable receiver for `Clock`'s
reason: the `Tee` it returns points into the struct's own array, so
calling it on a temporary has to be a compile error. The frames half
arrives already built rather than as a second directory because `DriverApp`
must not depend on Skia — it is used in builds that link none — while
the tree half is pure Zig and rides along with the module.

`skia.useFrame(app, size)` is the walk's first move, and
`skia.useMetrics(app)` is the same call at the size the app already has.
Both install the device measurer, set the viewport and lay out, so the
caller can read rects off the tree before taking a frame. What a walk
that skips them loses is *not* a stale-width frame — `capture`
re-lays-out, and both setters invalidate, so a frame is always
self-consistent with its own metrics. What it loses is agreement between
the frames and the **walk**: every step before the first capture was
hit-tested, asserted and nav-collapsed against the other measurer's
widths, and the size-dependent state entered back then — scroll offsets,
which tails folded — persists into the frames. Metrics and size up
front, unchanged for the whole walk, is what makes the trace a record of
one run. (The device measurer needs no warm-up for this: it loads the
bundled faces on its first measure rather than when a surface is
created, so a headless driver may lay out immediately.)
...
driver binary is not the app binary. It is one field on the call that
builds the driver
([getting-started.md](getting-started.md#the-fourth-artifact-a-driver)):

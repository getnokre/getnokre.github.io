...
[internals/dom-substrate.md](internals/dom-substrate.md#the-locale-axis-and-the-one-page-that-is-about-the-reader).
...
([the alternate set](internals/dom-substrate.md#the-alternate-set-and-the-two-writers-that-spend-it)).
...
`tr-`) fails the build where the catalog is read.

**The field set is closed at all three levels.** The top level admits
`@@locale`, `@@author`, `@@last_modified` and `@@context`, and nothing
else. A `@key` block admits `description`, `example`, `placeholders`,
`identical`, `sense`, and any `x-…` field of your own. A placeholder
admits `type`, `description`, `example`, and `x-…`. Any other name
fails the build naming the level, listing the set, and — where the name
is close to one of them — the field it is probably a typo of. Two
grounds, and [The refusals](#the-refusals) argues them: a typo was
undetectable, so `descriptoin` compiled and the sentence a translator
needed was gone with no diagnostic anywhere, and a misspelled `sense`
quietly reduced a glossary declaration to nothing; and a decode that
discards what it does not recognize cannot hand the catalog back byte
for byte, which is what a formatter stands on. `x-…` is the one open
namespace — carried verbatim through a decode and a re-encode, and read
by nothing here.

Each message may carry `@message` metadata, and only the template may
carry any of it at all
([What the compiler checks](#what-the-compiler-checks)). Of the closed
set, three fields are read: `placeholders`, `identical`
...
tooling, and `@@author`, `@@last_modified` and `@@context` are
provenance nokre carries and never interprets.
...
Latin-digits-in-Persian bug it exists to prevent. The DOM substrate
...
compile error until the CSS counter style is named, so the two substrates
...
one), so the titles and the catalog agree. A title function answers
from constant data — `tr` hands back exactly that — and must answer the
empty tag with words, the same footing as the empty `.fixed` title
`App.init` refuses. A chosen tag it answers with nothing is read as
that template answer, by `Title.text` and so by every reader of a
title: one destination in the template's words is what a hole in a
translation costs, and the switch itself never fails over it — a
catalog compiled through `l10n` cannot have the hole, since a missing
key is a compile error there, so what this covers is a title function
of your own and an empty message a catalog file carries. `setLocale`
refuses only a tag that is not one, and leaves the app exactly as it
was. The nav re-says itself on the spot: a row of destinations, a
...
- **A field nokre does not know.** The refusal names the level, the
  name, the closed set it is not in, and the declared field it is
  nearest to. `x-` is where a catalog carries something nokre should
  not read ([The format](#the-format)).
...
compiler can hold it. Six rules are not: whether a key anything
...
bytes somebody typed, whether a translated document still has its
source's shape, and whether every catalog beside the template is
written in the one layout. The first two need the app's sources, and
no `@import` reaches those — a check written as a test would have to
name every source file to read it, and the file it forgot is the one
hiding the defect. The third needs a vocabulary that is shared by more
than one package and therefore lives above any module root, where
`@embedFile` does not reach; and it could not move into the compiler
even if it did, because the same multi-needle scan over a real-scale
corpus cost 71.6 s and 2.0 GB at comptime against 0.27 s natively —
Zig's comptime is a tree-walking interpreter, and the search itself is
the cost. The last three need a *directory*: comptime can embed a file
it is given the name of and cannot walk a tree to find out what the
names are, so a rule whose subject is "every document under here" — or
"every catalog beside this one" — has no comptime form at all.
...
const app = nokre.addApp(nokre_dep, .{
    ...
    .l10n = .{
        ...
        .documents = nokre.pathList(b, &.{ b.path("src/content/articles"), b.path("src/content/docs") }),
    },
});
```

`pathList` copies a path list into the builder's arena: a `&.{…}`
literal written in a function that *returns* the declaration — which is
how an app declared for five targets writes it once — points into that
function's frame, and what reaches the check is whatever the frame
became.
...
compile. These six are about code being edited now: an orphaned key
...
- **Every key is referenced.** A key in the template that nothing under
  `sources` names is reported by name. A key is named by its enum literal
  ...
  **`sources` is a list, because an app is not always one tree.** An app
  assembled out of a shared package is two, and a key either of them
  names is named: the trees are read as one blob, so which tree holds
  the reference is no more a fact worth carrying than which file is.
  Every tree is walked and every `.zig` file under it is declared as an
  input, so a key that stops being named anywhere fails the next build
  of the app that defines it. An empty list is refused — it would orphan
  every key at once.
  ...
  **Deleting what it reports is a separate command, not a flag on this
  one** ([Purging unused keys](#purging-unused-keys)). This rule is
  about the edit in front of you; a purge is about a *decision* — that
  a surface is gone and its words go with it. The two cannot both be
  automatic, and the inversion is the non-obvious half: a build that
  repaired the orphan set would be a build that edits the tree, and
  this rule would never fire again, because the failure it exists to
  raise had already been quietly fixed. So it stays a hard failure
  precisely so that a purge has to be asked for — a warning nobody
  clears is how dead keys accumulate in the first place. And a
  destructive flag on the tool every `zig build` runs is one typo in a
  `build.zig` away from eating a catalog set. The consequence, stated:
  on a tree whose builds pass, a purge reports nothing.
...
- **Every catalog in the set is formatted.** A catalog is source, and
  source is formatted: the layout belongs to the formatter
  ([Formatting a catalog](#formatting-a-catalog)), so a catalog's diff
  is its content and never its shape — the same bargain `zig fmt`
  makes, and what lets a reviewer read a Persian draft against its
  English source two files in step. A file that differs from what the
  formatter would write is reported by name with the command that
  fixes it; nothing is written here, because a rule that edited the
  tree would be a build that edits the tree.

  This rule needs no path of its own. The set is the template's own
  siblings — the directory `.template` sits in, grouped by stem — so
  it turns on with `.l10n` itself and there is nothing further to
  declare. A second set in the same directory is another declaration's
  business and is not judged here; the formatter, given the directory,
  writes both.

## Formatting a catalog

`zig build l10n-fmt` writes the layout the rule above holds every
catalog to:

```
zig build l10n-fmt -- --dir examples/kitchen_sink/l10n
```

`--dir` formats every set in a catalog directory; `--input <file>`
formats one catalog; either may be repeated. `--check` writes nothing
and exits non-zero naming every file that would change, which is what
a human runs before committing.

**Nobody names the template.** `--input` finds the set its file
belongs to and derives the template from it, the same way
`translate-arb --dir` does
([Drafting a translation](#drafting-a-translation)) — asking for it
here would be a second answer to a question already settled.
`--template` overrides that for a file sitting outside a set — for one
`--input`, never for a `--dir` run, which identifies its own — and a
template needs none: its own key order is the authored one.

The layout itself is a handful of decisions, and these are the ones
worth knowing:

- **A template's key order is the author's, and it is kept.** Order
  encodes screen flow, so nothing sorts it
  ([The refusals](#the-refusals)).
- **A sibling is written in its template's order.** That is what makes
  a diff between two locales of one set line up line for line.
- **Every object is expanded**, `sense` with the rest, one field per
  line. The alternative was a per-field layout table for a single
  field.
- **A `\u` escape decodes to raw UTF-8.** Not cosmetic: it is what
  makes every catalog visible to the byte scanner, which names a
  `\u`-escaped file as the one corruption it cannot see
  ([src/l10n/encoding.zig](../src/l10n/encoding.zig)). The bytes are
  put through that scanner before the formatting and again before the
  write.
- **A directory may hold more than one catalog set.** Membership is by
  stem — a file's name minus the locale it declares, so
  `stub_fa_AF.arb` declaring `fa-AF` is stem `stub` — because two sets
  side by side is a shape a consumer's tree may have, and taking every
  `.arb` in a folder as one set levels each of them against the
  other's template.

Nothing here copies a byte of its input: every byte written is
produced from the decoded catalog, which is why the closed field set
([The format](#the-format)) is the precondition for the tool existing
at all. A `.arb` the reader will not take is a refusal naming the
file, never a file stepped over — a formatter that skips what it could
not read reports a pass with nothing behind it.

## Purging unused keys

Duplicate an app, split it in two, and delete from each half the
surfaces the other now covers: unused keys skyrocket. `zig build
l10n-purge` is what turns that report into one reviewable, undoable
commit, so that every translation the tree carries stays one that is
used.

```
zig build l10n-purge -- \
    --template src/l10n/app_en.arb \
    --src src \
    --catalogs src/l10n
```

`--src` is repeatable and means what `L10n.sources` means: every tree
read as one blob. An app whose keys are named from more than one tree
must be purged with all of them, and this is the one way to make purge
more aggressive than the check — pass the trees the app declares.

`--catalogs` defaults to the template's own directory, and `--label`
names the run in the report's first line — the `--src` path, if you do
not. **The default is a report**, and it leads with translations rather
than keys: a key is a row in a file, a translation is an act of
judgement somebody made, and the second number is the one being
protected. A report exits 0 whether or not it found orphans: the
build's own rule is the gate, and a second thing failing on one verdict
is a second home for it. `--require-none` is the third case — the same
report, and exit 1 if anything was found — for a caller that has to act
on the verdict and must not have to parse the prose to learn it; it
refuses `--write`, so deleting and asking whether anything is left are
two invocations. Every refusal below exits 1 on stderr instead.

`--write` performs the edit, and needs `--cut-dir <dir>` with it — what
it deletes is written back out as a catalog set, and nokre does not
pick a place inside your tree to put that. The directory is created if
it is not there. `--write` with nothing to delete is a refusal and not
a no-op, because a destructive run that wrote nothing is a run somebody
believes happened.

The report opens with what it *read* — the template, every catalog with
its key count, and how many `.zig` files were found — before any
verdict, so a run that read nothing cannot be mistaken for a run that
found nothing. It closes with what the namespace exemption alone is
holding: each namespace literal a source names, and how many keys
nothing but that literal is keeping alive. There is no per-file column,
because the rule reads the tree as one blob — reachability is from
anywhere, so which file holds the reference is not a fact it carries.

Five properties, each of which is a refusal somewhere:

- **It never cuts more than the build's own rule reports.** A purge
  runs the orphan rule above and adds nothing to it — same scan, same
  exemptions. A tool that second-guessed the rule would delete keys the
  next build demands back.
- **It refuses an unformatted set**, and names the formatter. What it
  keeps is re-encoded, so an unformatted input would arrive as reflow
  mixed into deletion and no reviewer could read the diff. Format
  first, commit that on its own, and every purge diff after it is
  nothing but deletions.
- **The set moves atomically** — every catalog or none. A half-purged
  set fails key parity, which is a build failure in every language at
  once rather than a quiet one in the language nobody reads.
- **It refuses a `--src` that is not a source tree.** Two shapes, both
  of which make every key an orphan and would turn `--write` into a
  catalog eraser: no `.zig` file under the path at all — asked of each
  tree, because a `--src` naming nothing is the same typo whether or
  not another one beside it read a thousand files — and every
  non-reserved key reading as an orphan. Structural, never thresholds —
  nokre carries no "more than N%" heuristic. `--force` is the override
  for the second, which an app being retired can reach honestly. The
  first has no override: there is no reading of "no source was read"
  under which a deletion derived from the sources means anything.
- **What it cut comes back as a catalog set**, under `--cut-dir`. That
  is a *transfer*, not a backup: the cut template keeps each key's
  `@`-metadata, so the entries can be moved into another app's
  template and compile there, while a retyped key loses the
  placeholder interface the whole bundle keys off. For the same reason
  the directory has to be empty — overwriting a cut set drops keys
  somebody was about to move into the other app — and `--force` is how
  you say you meant to. **Undo is git.**
  nokre invokes git nowhere — a library that started to would be
  making a claim about a consumer's version control it cannot keep —
  so the clean-tree precondition belongs to whatever runs the tool,
  the way it already does for `translate-arb`.

### Splitting one app into two

Format and purge each half, then read the two cut sets together. **A
split produces three answers and only the third is the one you
expected**, which is why purging one half alone is the mistake that
deletes a key the other half still needs — and that build failure
lands in the other package, later, in a language nobody was looking
at.

| cut from | means | do |
| --- | --- | --- |
| **both halves** | dead — nothing on either side of the split names it | delete; this is the split's actual garbage |
| **neither half** | shared — both halves still name it | a candidate for a common catalog; at minimum, two copies now kept in step by hand |
| **exactly one half** | it landed on the other side, which is the split working | nothing |

The last two rows are keys you **move**, which is what the cut set
being a real catalog set is for. Pairing two of them is the consumer's
question and not nokre's: nokre owns no notion of "the other app", so
it emits one machine-readable cut set per run and reading two of them
is a verb in the consumer's own tool.

One trap, and it is not a purge bug. A key stays exempt while a
literal naming it — or a namespace literal, a whole segment ending in
`.`, that covers it — survives anywhere under `--src`, and dead code
counts as anywhere: a screen nothing routes to keeps every key under
its prefix alive, in both tools. Delete the dead code first, then
purge. The keys nokre reserves for itself are never cut at all — they
have no literal in a consumer's tree to find, and cutting them would
turn a one-line re-enable into a re-translation.
...
- **No runtime catalog loading.** Catalogs ship inside the binary.
  ...
  version skew story, and a failure mode on first launch — three
  decisions handed back to every app that ships, to buy room a build
  already had.
...
- **No metadata field nokre does not know**, at any of the three levels
  ([The format](#the-format)). Two grounds, and the first is what the
  refusal is for. **A typo in a metadata key was undetectable**:
  `"descriptoin"` compiled, and its only symptom was a translator
  working without the sentence that explained the message — which
  surfaces as a bad translation months later, in a language the author
  cannot read. A misspelled `sense` was worse, because the glossary
  rule then read the declarations it could see and held nothing
  against the key that needed one. Second, **a field nokre ignores is a
  field a re-encode loses**: a formatter produces every byte from the
  decoded catalog, so a name the decode skips cannot be written back.
  Closing the set is what makes that round trip lossless by
  construction, rather than by a preservation list somebody has to
  keep in step. **This is a parameter, not a hole**: `x-` is the
  declared namespace, inside a `@key` block or a placeholder, and it
  round-trips untouched.
- **No `@@x-` extension namespace at the top level.** Nothing in three
  repositories ever used one, and admitting it would not have been an
  unused feature but a live bug: `@@` is the marker every other reader
  of an ARB uses to tell provenance from a message key, so a
  key-counting reader — a consumer's own, written in another language,
  deciding which catalog of a directory carries the metadata — reads
  `@@x-…` as metadata about a message and answers that question wrong.
  Declining the namespace deletes that hazard before it exists, and a
  field of your own goes one level down, where `x-` cannot be mistaken
  for anything.
- **No alphabetical sort of a template.** A template's key order is
  authored information — the order a reader meets the words, which is
  the screen's flow — and no other file carries that fact, so a sort
  would scatter it in a diff touching every line and buy nothing a
  reader wants: nobody looks a key up by scanning a catalog, they
  grep. The asymmetry is the part that looks inconsistent and is not: a
  template's order is authored and preserved, a sibling's is derived
  and enforced, and one is a decision where the other is its
  consequence. **This is a parameter, not a hole**: an author who wants
  alphabetical order writes the template that way, and the formatter
  keeps it.
...
| Unknown metadata field | Ignored | Build failure; `x-` is the declared extension namespace |
| Catalog layout | Whatever the author typed | The formatter's, and the build checks it |

...
```zig
    comptime { nokre.store_listing.requireListings(Strings); }
```
...
**Capture a page, not a desk.** A desk's regions abut the frame and each
draws a hairline on the edge it owns ([elements.md](elements.md)), which
is ink at x = 0 by design — so the scan reports a margin violation about
a screen that is laid out exactly as intended, and there is no number a
...
[dom-substrate.md](internals/dom-substrate.md), "The page's locale, not the
reader's".
...
the hook ([internals/dom-substrate.md](internals/dom-substrate.md)).
...
fn onNotification(state: *State, event: N.Event) void {
    switch (event) {
        ...
        // `switchToNearest`, not `navigate`: the payload may have been
        // written by the other app of this package, whose route table
        // is not this one's (routing.md, "Handing off from the other
        // app").

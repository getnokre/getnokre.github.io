...
What runs there is the **DOM substrate**: the same tree, written as
markup and drawn by the browser, in one 200 KB wasm module with no
Skia in it ([internals/dom-substrate.md](internals/dom-substrate.md)). It
...
- **The cursor is where screens are written; `append` is where
  ...
  *is* a `tree.append` call: a button with an empty label, a table row
  ...
  again; the raw `Tree` API stays public underneath for anything
  past those.
...
A product may be one package holding more than one app. Call `addApp`
again, share the `pkg`, and give the second one the other **dwell**:

```zig
_ = nokre.addApp(b, .{ .name = "notes", .root_source_file = b.path("src/main.zig"),
    .target = target, .optimize = optimize, .pkg = pkg });
_ = nokre.addApp(b, .{ .name = "notes-glance", .root_source_file = b.path("src/glance.zig"),
    .target = target, .optimize = optimize, .pkg = pkg, .dwell = .glance });
```

`dwell` is how long a reader is with the device the app is for —
`.sitting` (minutes or hours, a task with parts; the default, and what
every app in this course is) or `.glance` (seconds, one thing at a time,
usually with one hand busy). It is the **only** line that separates two
apps of a package, which is why two of them may not claim the same one.

What the two share is not a convention: it is the same code. The same
`State` type, the same action functions, the same catalog keys, the same
identity, icons and packaging manifests — so a label cannot drift and a
behavior cannot drift. What they do not share is the **route table**. A
glance screen is not a smaller copy of a page; its screens are
reorganized, and deciding that is authorship, which is exactly the thing
a library cannot do for you. `examples/hello` is the worked pair — one
`main.zig`, one `glance.zig`, one `State`, two tables.

Four things are refused at the build, each with the whole sentence:

- two apps of one package at the same dwell — the only line between them
  is the one thing there is to choose;
- one app declared for two targets saying two different things about
  which app it is;
- one package id with two identities, since every manifest generated
  from them would disagree;
- two store listings under one bundle id, because a store lists one.

**Which app runs on a device is which artifact you installed**, and that
was decided by which `addApp` call built it. There is no launcher and no
run-time choice: nokre reads `dwell` at build time only
([internals/architecture.md](internals/architecture.md)). A reference
that crosses from one to the other — a notification the glance app
scheduled, a deep link — arrives across a page load or a process launch
and is entered with `App.switchToNearest`
([routing.md](routing.md), "Handing off from the other app").
...
// build.zig routes every wasm32 target to the DOM substrate, and nokre
...
([internals/dom-substrate.md](internals/dom-substrate.md) argues each):
...
- **No fractional-scaling refusal.** A browser will hand out a 1.1
  device pixel ratio and no substrate can decline it on your behalf.

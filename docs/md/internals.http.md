...
- **Native.** A thread you can see, like a worker — but one-shot and
  *detached*: a socket mid-read cannot be interrupted (the
  ...
  a per-request watchdog waits out `deadline_seconds`, then claims
  ...
  cancelled one loses the generation check. It **waits on that claim,
  not on the clock**: the claim is the futex word the wait is parked on,
  so a request that answered in a millisecond ends its watchdog in a
  millisecond too, and only a request genuinely still on the wire leaves
  a thread behind. Sleeping the deadline out unconditionally, as it once
  did, left one thread per request holding that request's buffers for
  thirty seconds after it was over — which is what a driver freeing its
  process arena on the way out then freed underneath them. One
  consequence of detaching still
  ...
  must not run on a leak-checking allocator, and must not free the arena
  it ran on** — every one of those buffers is reported, and freeing the
  pages under a transfer that cannot be joined is a crash after a
  passing run. `testing.entry` settles both with a process arena it
  never deinits
  ([../testing.md](../testing.md#the-process-a-driver-is)); a driver
  that builds its own process shape answers it for itself.
  `native.atRest()` is how a gate asks whether any of those threads are
  left, since it may not join one
  ([native_test.zig](../../src/services/http/native_test.zig)).

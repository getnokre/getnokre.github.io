...
2. Layout rules in [layout.zig](../../src/core/layout.zig) — including
   ...
   direction-blind — don't touch it. A container the framework places
   **by role** rather than flowing — `region` is the one — still states
   its answer in `layoutBlock` *and* in `intrinsicSize`, beside the arm
   in its own pass: those two switches are what make "a well-formed tree
   never asks" true, and an element that skipped them would be relying
   on the reachability argument instead of stating it.
...
5. A11y mapping in [semantics.zig](../../src/a11y/semantics.zig), and its
   ...
   position (the DOM substrate takes no copy — it derives its roles in
...
8. Unit tests for each of the above (in the module's sibling
   ...
   check for it. The kitchen-sink entry is the one step an element can
   honestly fail: an element the sink cannot hold — one whose route must
   declare a shape the sink's routes do not — earns **another example**
   instead, standing the whole screen it belongs to. `region` is the
   precedent ([examples/support_console](../../examples/support_console/main.zig)),
   and an example is covered by having been added: the example gate
   walks the route table the app itself carries.
